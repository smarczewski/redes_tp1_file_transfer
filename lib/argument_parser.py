...
class ArgumentParser:
    ...
    def add_arguments(self):
        ...
        # Connection options
        self.parser.add_argument('-H', '--host', metavar='', default=DEFAULT_HOST,
                        help='server IP address')
        self.parser.add_argument('-p', '--port', metavar='', type=int, default=DEFAULT_PORT,
                        help='server port')
        ...
        self.parser.add_argument('-r', '--protocol', metavar='', default=False,
...
def get_args(argparser, app):
    ...
    if app == ParserType.SERVER:
        ...
        if not Path(argparser.args.src + '/' + argparser.args.name).exists():
            argparser.exit(1, message=f"ERROR: The file {argparser.args.name} doesn't exist\n")
        ...
        if Path(argparser.args.dst + '/' + argparser.args.name).exists():
            argparser.exit(1, message=f"ERROR: A file named {argparser.args.name} already exists\n")
